...
export default async function DashboardPage({ searchParams }: DashboardPageProps) {
  ...
  const [kpis, chartData, projectOptions] = await Promise.all([
    getDashboardKpis({ projectId, sessionUser }),
    getDashboardCharts({ projectId, sessionUser }),
  ...
  return (
    <section className="space-y-6">
      <FilterBar
        actions={
          <>
            <StatusBadge variant="danger">{kpis.customersOverdue} عميل متأخر</StatusBadge>
            <StatusBadge variant="warning">{kpis.openPromises} وعد سداد مفتوح</StatusBadge>
          </>
        }
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-2xl bg-surface-container-lowest p-5 ambient-shadow">
          <h3 className="mb-4 font-display text-title-lg text-on-surface">التحصيل حسب المشروع</h3>
          <CollectionByProjectChart data={chartData.byProject} />
        </div>
        <div className="rounded-2xl bg-surface-container-lowest p-5 ambient-shadow">
          <h3 className="mb-4 font-display text-title-lg text-on-surface">توزيع المتأخرات</h3>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.08fr_0.92fr]">
        ...
        <RecentFollowUps items={kpis.recentFollowUps} lastImportAt={kpis.lastImportAt} />
      </div>
    </section>
  );
}

async function getDashboardProjectOptions(sessionUser: Awaited<ReturnType<typeof getRequiredSessionUser>>) {
  ...
  return Array.from(new Set(visibleContracts.map((contract) => contract.project_id)))
    .map((id) => ({
      id,
